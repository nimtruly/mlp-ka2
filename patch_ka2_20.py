...
nb.cells[60] = nbformat.v4.new_markdown_cell(
    ...
    "The final model applies the four key techniques that actually move the score:\n\n"
    "1. **Threshold Optimization** — Instead of cutting at 0.5, we sort predictions by probability "
    "and mark the top ~22% as positive. This alone jumps the F1 score from ~0.62 to ~0.66.\n"
    "2. **Out-of-Fold Target Encoding on `last_name`** — Smoothing factor `m=20`, computed strictly "
    "out-of-fold (10-fold StratifiedKFold) to prevent target leakage.\n"
    "3. **HistGradientBoostingClassifier** — Single fast model. Trains in ~15 seconds and reaches "
    "the same score as any ensemble.\n"
    "4. **Simple Mean Ensemble (optional)** — If ensembling, use a plain average. Optimised weights "
    "overfit CV and drop on the leaderboard.\n"
)

# ── Cell 61: Feature preparation + OOF target encoding ────────────────────────
cell61 = '''\
# Prepare features for the final model
...
# Step 2: Out-of-Fold Target Encoding on last_name (smoothing=20, 10-fold StratifiedKFold)
...
# Full-train encoding for test set (no leakage risk — test labels are hidden)
...
train_final = pd.get_dummies(train_final, columns=["country", "gender"])
test_final  = pd.get_dummies(test_final,  columns=["country", "gender"])

# Drop identifier and target columns
drop_cols = ["id", "customer_id", "last_name", "exit_status"]
X_final      = train_final.drop([c for c in drop_cols if c in train_final.columns], axis=1)
X_final_test = test_final.drop([c for c in drop_cols if c in test_final.columns], axis=1)

# Align test columns to train
X_final_test = X_final_test.reindex(columns=X_final.columns, fill_value=0)
...
for fold, (train_idx, val_idx) in enumerate(skf_final.split(X_final, y_final)):
    ...
    # Step 3: HistGradientBoostingClassifier — fast, no tuning needed
...
# Step 1: Find best threshold — scan fine grid AND top-22% rank
best_thresh = 0.5
best_f1     = 0.0
...
# Also try top-22% approach (mark the highest-probability ~22% as 1)
...
cell63 = '''\
# Apply threshold to test probabilities and save submission
...
# ── Remove old cells 64 and 65 (now replaced / redundant) ─────────────────────
# Trim to 64 cells (0-63)
nb.cells = nb.cells[:64]

with open('ka2_20.ipynb', 'w', encoding='utf-8') as f:
    nbformat.write(nb, f)

print("ka2_20.ipynb patched successfully.")
