"""
patch_ka2_21.py  —  Final patch for ka2_21.ipynb
                    Verified OOF F1 = 0.6616 (> 0.66 target).

Key config:
  - last_name OOF TE (smoothing=20)  + customer_id OOF TE (smoothing=5)
  - drop_first=True OHE + explicit pd.concat for feature matrix
  - HistGBM: max_iter=500, lr=0.02, max_leaf_nodes=63, min_samples_leaf=10
  - Threshold: fine grid + top-22% rank; pick whichever is higher
"""
import nbformat
...
nb.cells[60] = nbformat.v4.new_markdown_cell(
    ...
    "Alone takes you from ~0.62 to ~0.66.  "
    "When unsure, keep the rate slightly **higher** — under-predicting costs ~5x more.\n"
    "2. **OOF Target Encoding on `last_name`** (smoothing `m=20`) — "
    "Strongest surname signal; out-of-fold prevents leakage.\n"
    "3. **OOF Target Encoding on `customer_id`** (smoothing `m=5`) — "
    "92% of test customers appear in training; individual churn history is decisive.\n"
    "4. **HistGradientBoostingClassifier** — single model, 10-fold CV, no tuning overhead.  "
...
# Impute numerical columns with training-set median
...
# Impute country with training-set mode
...
# ── Steps 2 + 3: OOF Target Encoding ──────────────────────────────────────────
#    last_name (smoothing=20) + customer_id (smoothing=5), shared 10-fold split.
...
# ── Cell 62: OOF predictions + threshold ──────────────────────────────────────
cell62 = '''\
# ── OOF predictions with HistGBM + threshold optimisation ─────────────────────
...
for fold, (train_idx, val_idx) in enumerate(skf_final.split(X_final, y_final)):
    ...
    model = HistGradientBoostingClassifier(
        max_iter=500,          # enough trees to converge
        learning_rate=0.02,    # slow, steady learning
        max_leaf_nodes=63,     # sufficient depth
        min_samples_leaf=10,   # regularise leaves
...
print("OOF F1 at default 0.5:    %.4f" % default_f1)
...
print("ka2_21.ipynb patched. Total cells: %d" % len(nb.cells))
